...
export const enhancedExport = async (
  ...
  toast.info(`Preparando a exportação no formato ${format.toUpperCase()}...`);
  ...
  try {
    ...
    if (requiredFields.length > 0) {
      ...
      if (invalidRows.length > 0) {
        toast.warning(`${invalidRows.length} linha(s) ignorada(s) pois campos obrigatórios estão faltando`);
      }
    }
    ...
    if (validateRow) {
      ...
      if (validData.length < data.length) {
        toast.warning(`${data.length - validData.length} linha(s) ignorada(s) devido à validação personalizada`);
      }
    }
    
    if (validData.length === 0) {
      toast.error("Nenhum dado válido após validação");
      return false;
    }
