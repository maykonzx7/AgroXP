...
export const generatePreviewHTML = (
  ...
  if (!data || data.length === 0) {
    return '<div class="p-4 text-center">Nenhum dado disponível para visualização</div>';
  }
      ...
      col => {
        ...
        if (isLink) {
          if (value.includes('@')) {
            ...
            return `<td class="px-4 py-2 border-b dark:border-gray-600"><a href="${value}" class="text-blue-600 dark:text-blue-400 hover:underline">Ver detalhes</a></td>`;
          } else {
            return `<td class="px-4 py-2 border-b dark:border-gray-600"><a href="${value}" target="_blank" rel="noreferrer" class="text-blue-600 dark:text-blue-400 hover:underline">Link externo</a></td>`;
          }
        }
  ...
  return `
    <div class="p-6 dark:bg-gray-800 dark:text-gray-100">
      <h2 class="text-xl font-bold mb-4">${title || `Visualização - ${moduleName}`}</h2>
      ...
      <div class="mt-6 text-sm text-gray-500 dark:text-gray-400 text-right">
        <p>Data: ${new Date().toLocaleDateString(locale || 'pt-BR')}</p>
      </div>
      ${navigationButtons}
    </div>
  `;
