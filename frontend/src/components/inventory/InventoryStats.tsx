...
const InventoryStats: React.FC<InventoryStatsProps> = ({
  ...
  const getLowStockItems = () => {
    return inventoryData.filter((item) => item.quantity <= item.minQuantity)
      .length;
  };
  ...
  const getInventoryHealthStatus = () => {
    ...
    if (lowStockItems / totalItems < 0.1)
      return { label: "Bom", color: "text-lime-500" };
    if (lowStockItems / totalItems < 0.25)
      return { label: "Médio", color: "text-amber-500" };
    ...
    <div className="space-y-6 animate-enter">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="bg-white rounded-xl border">
          ...
          <CardContent>
            <p className="text-3xl font-bold">R$ {getTotalInventoryValue()}</p>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="flex flex-col">
              ...
              <p className="text-muted-foreground">
                {getLowStockItems()} para reabastecer
              </p>
            </div>
          </CardContent>
        </Card>
