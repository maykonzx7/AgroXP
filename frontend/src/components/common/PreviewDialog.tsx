  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl h-[80vh]">
        <DialogHeader>
          <DialogTitle>{title || `Visualização - ${moduleName}`}</DialogTitle>
          <DialogDescription>
        ...
        <div className="flex-grow overflow-auto border rounded-md mt-4 bg-background">
          <iframe
            srcDoc={`
              <!DOCTYPE html>
              <html lang="${settings.locale || 'pt-BR'}">
                <head>
                  <meta charset="UTF-8">
                  <meta name="viewport" content="width=device-width, initial-scale=1.0">
                  <title>${title || `Visualização - ${moduleName}`}</title>
                  <style>
                    :root {
            ...
            title="Preview"
            aria-label={`Visualização do documento: ${title || moduleName}`}
          />
        </div>
