// Script para executar seed da conta de admin
...
async function main() {
  ...
  // Garantir que temos pelo menos os campos necessários
  const field1 = fields[0];
  const field2 = fields[1] || fields[0]; // Fallback para field1 se não existir
  const field3 = fields[2] || fields[0]; // Fallback para field1 se não existir
  const field4 = fields[3] || fields[0]; // Fallback para field1 se não existir
  const field5 = fields[4] || fields[0]; // Fallback para field1 se não existir
  const field6 = fields[5] || fields[0]; // Fallback para field1 se não existir
  ...
  let crops;
  if (existingCrops.length === 0 && fields.length > 0) {
    // Usar apenas os campos disponíveis
    const availableFields = fields.slice(0, 6); // Máximo de 6 campos
    const cropData = [
      {
        name: "Soja",
        variety: "BRS 284",
  ...
  // Garantir que temos pelo menos os campos necessários
  const livestock1 = livestock[0];
  const livestock2 = livestock[1];
  const livestock3 = livestock[2];
  const livestock4 = livestock[3];
  const livestock5 = livestock[4];
  const livestock6 = livestock[5];
  const livestock7 = livestock[6];
  ...
  if (existingInventory.length === 0) {
    ...
    console.log(`✅ ${existingInventory.length} itens de inventário já existentes\n`);
  }
